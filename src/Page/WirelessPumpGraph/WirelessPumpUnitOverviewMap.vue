<template >
  <div class="wp-UnitOverviewMap" ref="ui">
    <div class="graphs" id="lefttupu"  >
      <div class="ingraphs" v-if="overviewType === '振动概貌图'" ref="needmid"  :style="{height: mapheight,backgroundColor: nowImageBackground, width:'100%'}"  id="midtupu" @contextmenu.prevent.stop="handleRightClick($event)">
        <UnitOverviewGrap ref="unitOverview" :nowtimeStatic="nowtimeStatic"  :get-scroll-value="getScrollValue" @graphImageBackground="graphImageBackground"
                          ...
                          @datanull="datanull" @datafail="datafail"></UnitOverviewGrap>
      </div>
      <OverViewIframe
        class="other-graph"
      ...
      <div class="divGraph" :style="{height: graphheight}">
        <TrendGraph ref='trendg' v-show="show" @historyInformation="historyInformation"
                    @chartinfo="chartinfo" @iscan="myiscan"
                    :nowtimeStatic="nowtimeStatic"  @refine="refine" @cancelRefine="cancelRefine"
                    @dataZoom="dataZoom" @reDataZoom="reDataZoom"
        ></TrendGraph>
      </div>
    </div>
    <!--  隐藏操作页按钮  -->
    ...
    <!--    <div id="menu" v-show="isMenuShow" :style="{top:y+'px',left:x+'px'}">-->
    <!--      <div class="menu" @click = 'handleJump("/UnitOverviewMap")' >机组概貌图</div>-->
    <!--      <div class="menu" @click = 'handleJump("/AlarmQueryGraph")' >机泵报警查询</div>-->
    <!--      <div class="menu" @click = 'handleJump("/TrendAnalysis")' >趋势分析</div>-->
    <!--      <div class="menu" @click = 'handleJump("/PumpImpactDiagnosis")' >冲击诊断</div>-->
    <!--      &lt;!&ndash; <el-menu-item index="VibrationMonitor" v-show="menu.SpecialManualOfPump.data['3']">振动监测</el-menu-item>&ndash;&gt;-->
    <!--&lt;!&ndash;      <div class="menu" @click = 'handleJump("RotorFaultDiagnosis")'>转子类故障诊断</div>&ndash;&gt;-->
    <!--&lt;!&ndash;      <div class="menu" @click = 'handleJump("CepstrumGraph")'>倒谱图</div>&ndash;&gt;-->
    <!--      &lt;!&ndash;   <el-menu-item index="ChangeVisualization" v-show="menu.SpecialManualOfPump.data['5']">变工况可视化</el-menu-item>&ndash;&gt;-->
    <!--&lt;!&ndash;      <div class="menu" @click = 'handleJump("SingleMultiBarValueChart")'>单多值棒图</div>&ndash;&gt;-->
    <!--      &lt;!&ndash;  <el-menu-item index="ComprehensiveAnalysis" v-show="menu.SpecialManualOfPump.data['7']">综合分析</el-menu-item>&ndash;&gt;-->
    <!--      &lt;!&ndash;<el-menu-item index="MultiParametersTrendGraph" v-show="menu.SpecialManualOfPump.data['8']">多参数趋势图</el-menu-item>&ndash;&gt;-->
    <!--      <div class="menu" @click = 'handleJump("OtherParametersTrendGraph")'>其它参数趋势图</div>-->
    <!--    </div>-->
  </div>
</template>
...
import AlarmTrendGraph from "../../components/ComUtils/AlarmTrendGraph.vue";
import keyPressSignalMixin from '@/mixins/keyPressSignalMixin';
export default {
  name: "WirelessPumpUnitOverviewMap",
  components: {AlarmTrendGraph, UnitOverviewGrap, TrendGraph, UnitOverviewMapOpPage, OverViewIframe, OpHideButton},
  ...
  watch:{
    ...
    show1() {
      // console.log('问题在这',this.showOpPage ,this.$store.state.showOp)
      // if(this.showOpPage == true && this.$store.state.showOp == true){
      //   this.newwid = (window.screen.width - 600) + 'px'
      // }
      // else if(this.showOpPage == false &&  this.$store.state.showOp == true){
      //   this.newwid = (window.screen.width - 250) + 'px'
      // }
      // else if(this.showOpPage == true &&  this.$store.state.showOp == false) {
      //   this.newwid = (window.screen.width - 310) + 'px'
      // }
      // else {
      //   this.newwid = (window.screen.width - 80) + 'px'
      // }
    },
    topiccolor(val) {
      this.mycolor = this.$store.state.textcolor;
  ...
  methods: {
    ...
    imageWidth(args){
      ...
      // this.imageWidths=(Number(args.split('px')[0])-w)/2;
      //  console.log('??1')
      //  console.log(args)
      //  console.log(w);
    ...
    imageheights(args){
      ...
      //console.log('??2')
      //this.nowimageheights=(Number(args.split('px')[0])-h)/2;
      // console.log(args);
      // console.log(h)
      // console.log( this.nowimageheights)
    },
    gomid() {
      //console.log('121212',this.imageWidths,this.nowimageheights)
    ...
    dataZoom(zoomBegin,zoomEnd){
      if ( zoomBegin != null && zoomEnd != null){
        ...
        //   console.log("asdadadaklsdakldassdkljajdkl",this.$refs.trendgs)
    ...
    reDataZoom(){
      ...
      //     this.$refs.trendgs[i].reDataZoom(reOriginalSeries3,reOption3,this.zoomDataIntervalBegin[this.zoomDataIntervalBegin.length - 1],this.zoomDataIntervalEnd[this.zoomDataIntervalEnd.length - 1])
      //   }
      // }
    ...
    nowstatic(data) {
      ...
      //    this.$store.commit('nowtimePath',this.$route.path );
        ...
        this.$axios.get("/graph/" + equipmentUuid1 + '/' + nowtime + "/historyData")
          .then(res => {
            ...
            if (res.data.code === 200) {
              ...
              // this.rev = res.data.data[0].trend.rev + res.data.data[0].trend.unit
              // console.log('0000000000000000');

              // console.log(this.rev)  //转速
    ...
    iDrawoutherrealGrap(equid, pointids, mytree, feature) {
      this.setinterval();
      this.DrawoutherrealGrap(equid, pointids, mytree, feature)
    },
    DrawoutherrealGrap(equid, pointids, mytree, feature) {
      ...
      this.$axios.get("/proctrend/" + equipmentUuid1 + "/" + pointIdString1 + "/rt")
        .then(res => {
          if (res.data.code === 200) {
            ...
            if (title in this.channelType && this.channelType.hasOwnProperty(title)) {
              // 如果title是对象的属性，返回对应的值
              title =  this.channelType[title];
            }
    ...
    iDrawrealgrap(equid, pointids, mytree, feature){
      this.setinterval();
      this.Drawrealgrap(equid, pointids, mytree, feature)
    },
    Drawrealgrap(equid, pointids, mytree, feature) {
      this.$emit('dataloading');
      this.tongdao = 0;
      this.graphtype = '实时'
      this.nowtree = mytree;
      ...
      this.$axios.get("/trend/" + equipmentUuid1 + "/" + pointIdString1 + "/real_time")
        .then(res => {
          ...
          if (res.data.code === 200) {
            ...
            var title = this.features[mytree[0].children[0].type][feature].label;
            var newX = timestampToTimeFormat(this.chartdata.trendTime);

            var flagX = '[时间]';
            // var flagY = '[um]';
            var flagY = '[' + "unit" + ']';
            // console.log("unit",this.nowtree[0].children[0].type)
            var  flagY='[' + this.features[this.nowtree[0].children[0].type][feature].unit + ']' ||'[]';
            var dataX = [];
            var dataY = [];
            dataX.push(newX);
    ...
    DramHistorygrapOther(equipmentUuid, pointIdString, stime, etime, mytree, feature) {
      ...
      this.$axios.get(netinfor, {
        ...
        if (res.data.code == 200 && res.data.data.trendInfo.length != 0) {
          ...
          this.$emit('datasuccess');
          this.chartdata = res.data.data;
          // var title = this.chartdata.equipmentName;
          var title = this.features[mytree[0].children[0].type][feature].label;
          ...
          if(dataX.length == 0) {
            ...
            this.$refs.trendg.setchart(flagX, flagY, dataX, dataY, title, "history");}
          // var myinfos = {trendTime: trendinfo[i].trendTime};
          //  this.historyInformation(myinfos);
        } else {
          this.clearchart();
          this.$emit('datafail')
          //this.$message.error(res.data.message);
    ...
    DramotherHistoryGrapOther(equipmentUuid, pointIdString, stime, etime, mytree, feature) {
      ...
      this.$emit('dataloading');
      // console.log(this.nowtree);
      // console.log('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', equipmentUuid, pointIdString, stime, etime, mytree, feature)
      this.nowtree = mytree;
      this.lastpointsid=pointIdString
      this.lastfeature=feature;
      //   console.log(this.nowtree)
      ...
      this.$axios.get(netinfor).then(res => {
        ...
        if (res.data.code == 200 && res.data.data != null) {
          ...
          var dataX = [];
          var dataY = [];
          // var title = this.$store.state.equName + ' '
          var title = mytree[0].label;
