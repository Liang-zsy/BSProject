<template>
  <div class="trend-analysis-op-pagejibeng" ref="ui">
    ...
    <div class="Reciprocat-opPage-body">
      <el-form ref="ruleForm" :label-width="labelWidth" class="demo-ruleForm" size="mini">
        <el-form-item :label="translateText('opPage.textMap.','设备名称:')">
          <span style="font-size: 14px">{{getequName}}</span>
        </el-form-item>
        ...
        <div>
          ...
          <select-number class="selectnumber" v-if="nowstatus == 2"></select-number>
        </div>
        <el-form-item>
...
import SelectGraph from "../ComUtils/SelectGraph.vue";
import TrendAnalysisPointTree from "./TrendAnalysisPointTree";
...
export default {
  ...
  watch: {
    equid(){
      /* 111行的代码if(!this.activatedFlag) return;
         这是唯一一个在操作页引用了keepAliveMixin的activatedFlag变量的，
         这行代码会导致同类型设备之间切换时测点和图谱显示不一致，所以我暂时把它注释了
      */
      if(!this.activatedFlag) return;
      // if(!this.activatedFlag) return;
      // this.firstFlag = true;
      if(this.$route.path == '/TrendAnalysis') {
        this.firstFlag = true;
      }
    },
    topiccolor(val) {
    ...
    FreshHistoryGraph() {
      ...
      if (this.mytrees.treeSelect == 0) {
        if (this.mytrees.checkedNodes.length <= 1) {
          ...
          console.log("趋势分析测点树",this.mypointid,this.mytrees);
